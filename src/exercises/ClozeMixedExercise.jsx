...
export async function generateClozeMixed(topic, count = 2) {
  const system = `Generate Spanish cloze passages with multiple-choice options for each blank. Each passage should be 3-5 paragraphs long (approximately 150-250 words) with 8-16 meaningful blanks strategically placed throughout the text (maximum two per sentence).
...
Provide a clear student instruction as a separate field named studentInstructions. Do not include the instruction text inside the passage itself.`;
  ...
  const user = `Create exactly ${count} cloze-with-options passages about: ${topic}. Each passage should be substantial (3-5 paragraphs) with 8-16 blanks (maximum two per sentence), each having 4 options. Remember: each blank must use exactly 5 underscores (_____).`;
